...
Recall a \textit{book-embedding} of a graph \(G\) is an embedding of \(G\) on a book. Further recall the \textit{pagenumber} of a graph \(G\) is the minimum number of pages of a book required to embed \(G\).
