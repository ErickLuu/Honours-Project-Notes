...
Subdivide $K_n$ $n$ times. \textcite{atneosenEmbeddabilityCompacta$n$books} proved that this subdivision of $K_n$ can be embedded on three pages. But $K_n$ is a minor of its subdivision, and from \cref{thm:Pagenumber_Complete_Graph}, $\pn(K_n) = \lceil \frac{n}{2} \rceil$. 

\begin{theorem}
	There exists a family of 2-degenerate graphs with unbounded pagenumber.
\end{theorem}
\begin{proof}
	Let $G_n$ be the complete graph $K_n$ with every edge subdivided once. Then $G_n$ is bipartite, so is $2$-colourable, and $2$-degenerate. \textcite{eppsteinSeparatingThicknessGeometric2002} showed that for every $t$ there exists an $n$ such that $G_n$ cannot be embedded in $t-1$ pages. This proof uses \textit{Ramsey theory}.
\end{proof}
