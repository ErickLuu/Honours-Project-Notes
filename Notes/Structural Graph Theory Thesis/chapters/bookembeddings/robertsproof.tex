...
This section discusses tree-decompositions and book-embeddings of graphs where every torso in the tree-decomposition has a bounded book-embedding. \textcite{hickingbothamStackNumberCliqueSum2023} proved that every graph with a tree-decomposition where every torso is embeddable on $s$ pages can be embedded on $2s^2 + 4s + 1$ pages. This proof can be seen as an extension of \cref{thm:bded_treewidth_bded_pagenumber}, where each torso has bounded pagenumber, rather than a bounded number of vertices. This proof is then applied to planar graphs, using a lemma that every planar graph has a tree-decomposition where every torso is either $K_{\leq 4}$ or a $4$-connected planar graph. 
...
\begin{theorem}[By \textcite{hickingbothamStackNumberCliqueSum2023}]\label{thm:clique_sum_pagenumber_bound}
	...
	Restricting $(<, \psi)$ to $B_x$ yields the same book-embedding as $<_x$, except the adhesion set $B_x \cap B_y$ is moved to the start of the book-embedding, and each vertex in $B_x \cap B_y$ are given at most $\ell$ new colours. 
	
	Furthermore, if the adhesion set $B_x \cap B_y$ is consecutive in $<_x$, then $<_x = <$ up to a circular ordering.
\end{theorem}

To prove \cref{thm:clique_sum_pagenumber_bound}, the hypothesis in \cref{thm:clique_sum_pagenumber_bound} is strengthened so that induction can be applied, in \cref{lem:Hickingbotham_Lemma}. Then given a tree-decomposition, a leaf is removed. Then induction is done on the rest of the tree. Finally, the leaf is added back in so that the stronger induction hypothesis can be shown to hold. 
...
\begin{lemma}\label{lem:Hickingbotham_Lemma}
	...
	If $C$ is consecutive in $\leq_a$, then $\leq_a$ equals $\leq$ up to a circular ordering.
\end{lemma}
\begin{proof}
	...
	If $C$ is consecutive in $\leq_a$, then $C$ is at the start of the ordering. This is done by rotating $\leq_a$. 
...
\begin{theorem}[\textcite{hickingbothamStackNumberCliqueSum2023}]
	...
	Furthermore, if the adhesion set $B_x \cap B_y$ is consecutive in $<_x$, then $<_x$ equals $<$ up to a circular ordering.
\end{theorem}
