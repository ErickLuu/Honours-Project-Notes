...
Let $G$ be a plane graph. A \textit{separating triangle} on $G$ is a triangle that separates $\mathbb{R}^2$ into two components $U, V$ and there exist vertices in both the interior of $U$ and $V$. Every nonseparating triangle bound a face. \cref{fig:separating_triangle} illustrates an example of a separating triangle. 
\begin{figure}[h!]
	\centering
