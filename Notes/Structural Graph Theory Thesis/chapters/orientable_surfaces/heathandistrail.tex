...
\end{lemma}
\begin{proof}
	Use \label{thm:Planar Graph Hickingbotham Bound} to embed $G_P$ on four pages with the property that the outer-cycle remains in order with respect to the embedding. For each homotopy class in \(E_P\), allocate three pages. One page is for vertices in the same block, and the other two pages are used for edges between blocks. Therefore, \(11 + 3(6g - 3) = 18g + 2\) pages suffice if \(G\) has a planar-nonplanar decomposition.
\end{proof}