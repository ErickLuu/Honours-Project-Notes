...
From \cref{thm:planar_graph__be_mono_paths}, every planar graph can be embedded on $11$ pages. However, Yannakakis gives a tight bound, as there exist planar graphs that need four pages, from \textcite{yannakakisPlanarGraphsThat2020,bekosFourPagesAre2020}. 
...
\begin{theorem}[\textcite{heathPagenumberGenusGraphs1992}]\label{thm:Genus_pagenumber_bound}
	When $g \geq 1$, every orientable genus $g$ graph can be embedded in $O(g)$ pages.
\end{theorem}
...
\begin{enumerate}
	\item Every edge in \(E_N\) is between vertices on the boundary of $F_0$, the outerface of $G_P$.
...
Think of \(G_n\) as living on a disk on \(S\). Taking \(G_n\) to a point, \(u_1v_1\) and \(u_2v_2\) are homotopic if and only if the two loops are homotopic in a topological sense.
