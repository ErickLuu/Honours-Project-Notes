...
However, we conjecture that the only counterexamples are graphs with faces that look like \cref{fig:moved_vertices}, up to removing cut-sets of sizes $1$ and $2$. Notice how any two faces have at most two triangles in common, because if there were three triangles in common, there would be a cutset of size two. We conjecture that there is a way to apply \textcite{thomassenTheoremPathsPlanar1983} to only have one vertex out of the cutset $C$ to be moved out of position, and that every face has a bounded number of monochromatic paths. 
