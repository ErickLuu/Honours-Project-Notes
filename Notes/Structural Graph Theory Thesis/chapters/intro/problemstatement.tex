...
The second goal is to attempt to solve \cref{conj:bded_had_pn}. We aimed to prove that every planar graph can be embedded on $s$ pages with some properties that allow us to embed a vortex on a face, but we were unable to prove the main theorem. The main roadblock was assuming that \cref{thm:planar_graph__be_mono_paths} implies that every planar graph has a book-embedding with a bounded number of monochromatic paths. However, this was not the case. Graphs embedded on orientable surfaces is discussed in \cref{chap:orientable} and graphs embedded on the projective plane is discussed in \cref{chap:nonorientable}. We also provide some proof directions for the case when $\Sigma$ is the Klein bottle in \cref{chap:nonorientable}. 

We also state a conjecture that every graph embeddable on a surface of Euler genus $g$ can be embedded on $f(g)$ pages where every face has a bounded number of monochromatic paths. If this conjecture is true, then it would imply \cref{conj:bded_had_pn}.  This is discussed in \cref{chap:Future Work}. We had previously assumed that this conjecture holds in the case of planar graphs. In fact, this conjecture holding for planar graphs implies it holds for graphs embedded on orientable surfaces and the projective plane. However, the book-embedding devised does not satisfy this conjecture. We discuss other attempts to solve this theorem, in \cref{chap:planar}.