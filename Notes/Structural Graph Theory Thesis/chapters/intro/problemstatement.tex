...
The second goal is to attempt to solve \cref{conj:bded_had_pn}. We prove that when a surface $\Sigma$ is orientable or the projective plane, some graphs which are almost-embeddable on a surface of Euler genus $g$ with $p$ vortices of depth $k$ on some faces are embeddable in $f(g, p, k)$ pages. The case when $\Sigma$ is orientable is discussed in \cref{chap:orientable} and the case when $\Sigma$ is the projective plane is discussed in \cref{chap:nonorientable}. We also provide some proof directions for the case when $\Sigma$ is the Klein bottle in \cref{chap:nonorientable}. 

We show that every $K_6$-minor-free graph can be embedded on a bounded number of pages. We also state a conjecture that every graph embeddable on a surface of Euler genus $g$ can be embedded on $f(g)$ pages where every face has a bounded number of monochromatic paths. If this conjecture is true, then it would imply \cref{conj:bded_had_pn}. This is discussed in \cref{chap:Future Work}.