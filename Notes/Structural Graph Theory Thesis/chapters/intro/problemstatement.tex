...
The second goal is to attempt to solve \cref{conj:bded_had_pn}. We have some directions when $\Sigma$ is orientable, from \textcite{heathPagenumberGenusGraphs1992}. The case when $\Sigma$ is orientable is discussed in \cref{chap:orientable} and the case when $\Sigma$ is the projective plane is discussed in \cref{chap:nonorientable}. We also provide some proof directions for the case when $\Sigma$ is the Klein bottle in \cref{chap:nonorientable}. 
