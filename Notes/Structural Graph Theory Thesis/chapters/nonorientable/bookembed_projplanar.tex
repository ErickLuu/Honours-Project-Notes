...
We prove a weaker version of the above theorem.

\begin{theorem}\label{thm:proj_planar_graphs_23pages}
	Every graph embedded on the projective plane has a book-embedding with 23 pages, where every face has $24$ monochromatic paths. 
\end{theorem}
...
\begin{figure}[h]
    \centering
    \includesvg[width = 0.5 \textwidth]{figures/projectiveplanar_decomposition.svg}
...
Now we will prove \cref{thm:proj_planar_graphs_23pages}. This is done by breaking the disk into two halves and finding a book-embedding where the non-orientable edges are nested.
\begin{proof}
    ...
    From \cref{thm:planar_graph__be_mono_paths}, there exists a 11-page embedding $(<_1, \sigma_1)$ of $D_1$ which preserves the cycle $P \cup B_1$. Similarly, there exists a 11-page embedding $(<_2, \sigma_2)$ of $D_2$ which preserves the cycle $P \cup B_2$.
    Let $t$ be at the start of the vertex ordering of $<_1$ and $s$ be at the end of the vertex ordering of $<_2$. 
    Combine these two embeddings along $P$ (interlace $<_1, <_2$ along $P$) to form a book-embedding $(<, \sigma)$ of $G_P$ with 22 pages. This is a vertex ordering of every edge in $G$ as $G_P$ is spanning. 
    ...
    This embeds a projective-planar graph in 23 pages.

    Furthermore, each face transitions from planar to nonplanar edges $4$ times, from \cref{corr:orientable_nonplanar_faces}, and there are 24 monochromatic paths on each face of $G_P$. Then each face of $G$ has at most $4(24 + 1) = 100$ monochromatic paths.
\end{proof}
