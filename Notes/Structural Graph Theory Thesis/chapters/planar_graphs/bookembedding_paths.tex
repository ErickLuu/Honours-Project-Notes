\section{Path-decompositions and vortices}

In this section, we attempt to find some graph parameter that allows us to bound the number of pages necessary to embed a vortex on a face.
...
\begin{claim}[Incorrect claim]\label{conj:fsk_be}
...
\begin{corollary}\label{corr:fsk_be}
	Suppose \cref{conj:fsk_be} holds. Let $G$ be a graph, where $G = G_0 \cup G_1 \cup \ldots \cup G_p$. Furthermore, $G_0$ is embedded on a surface $\Sigma$ with Euler genus $g$, and $G_i$ is a vortex on $G_0$ on a face $F_i$ of width $k$. If every graph embedded on $\Sigma$ has a book-embedding on $s$ pages, there exists a $p f$ book-embedding of $G$, where $f = f(s, k)$.
\end{corollary}
...
\cref{corr:fsk_be} implies \cref{conj:bded_had_pn} in the case when $\Sigma$ is an orientable surface or projective plane, using \cref{lem:planar_nonplanar_orientable} and \cref{thm:proj_planar_graphs_23pages} through \cref{thm:apex_vertices_pagenumber} and \cref{thm:gmst}. 
...
\begin{figure}[h!]
	\centering
	\includesvg[inkscapelatex=false, width = 0.5\textwidth]{figures/counterexample2.svg}
