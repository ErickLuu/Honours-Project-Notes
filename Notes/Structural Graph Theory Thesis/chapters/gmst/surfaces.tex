...
The \textit{Euler Genus} of a \textit{graph} \(G\) is the smallest Euler genus \(g\) surface \(\Sigma\) such that \(G\) can be $2$-cell embedded on $\Sigma$. Euler genus is a graph invariant, $\eg(G) = g$. By the classification of surfaces, $\eg(G) = \min(2 \gamma(G), \tilde{\gamma}(G))$. 
