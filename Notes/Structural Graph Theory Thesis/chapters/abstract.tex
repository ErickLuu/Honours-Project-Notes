...
\begin{abstract}
	...
	We utilise Robertson and Seymour's Graph Minor Structure Theorem to obtain results on $K_t$-minor-free graphs. This theorem states that every $K_t$-minor-free graph can be built up from four ingredients: graphs embedded on surfaces, vortices, apex sets, and clique-sums. 
	This thesis is an attempt to apply the Graph Minor Structure Theorem to show that every $K_t$-minor free graph can be bounded on $f(t)$ pages. We prove some properties about planar graphs and vortices on planar graphs. We discuss some attempts to find a book-embedding of a planar graph on a bounded number of pages that allows us to add on a vortex without blowing up the pagenumber. 
