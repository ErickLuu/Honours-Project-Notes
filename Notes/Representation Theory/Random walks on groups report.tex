...
\end{proof}
We have another result by Plancherel, which we lift from Serre.
...
In the paper by Diaconis and Shahshahani\cite{diaconisGeneratingRandomPermutation1981}, he bounds this constant above by looking at each cycle length and creating ``zones'' which partition the set of partitions of $n$. However, we can bound it above by a constant which tends to zero as $k$ goes to infinity. 
