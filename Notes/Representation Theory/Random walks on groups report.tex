...
A random walk on a group $G$ driven by $P$ is said to be ergodic if there exists an integer $N > 0$ such that $P^{\ast n}(g) > 0$ for all $n > N$ and $g \in G$. This means that after some point in time, the probability of reaching all states is nonzero after a certain burn-in period.
...
\begin{definition}
	...
	\end{equation}
\end{definition}
This is almost the Fourier transform, which we will see later. Like the Fourier transform, it converts convolutions to multiplication.
...
\end{theorem}
The proof given below is essentially the same proof from \cref{lem:rep_ast}.
\begin{proof}
	We have that:
\begin{align*}
...
\end{proof}
We have another result by Plancherel, which we lift from Serre \cite{serreExamples1977}. This example is used to bound above the rate of convergence.
...
Therefore, we have that $\widehat{P}(\chi_Y)$ has the elements $y \in Y$ contribute $ -1/n$ each to $\widehat{P}(\chi_Y)$ and the elements not in $Y$ contribute $1/n$ to $\widehat{P}(\chi_Y)$. Taking the sums, we have that the spectrum must be $\frac{1}{n}(n - 2 |Y|) = 1 - \frac{2|Y|}{n}$. As there are $\binom{n}{j}$ subsets $Y$ where $|Y| = j$,
...
Now we use the fact for when $j > \left\lfloor \frac{n}{2} \right\rfloor$, we have the exact same numbers, so we exploit this symmetry and only calculate the left half of the summation to yield:

\begin{equation}
	\left\| P^{\ast k} - U \right\|_{TV}^2 \leq \frac{1}{2} \sum_{j = 1}^{\left\lfloor \frac{n}{2} \right\rfloor}\binom{n}{j} \left[1 - \frac{2j}{n}\right]^{2k}
\end{equation}
Now we have that $\binom{n}{j} \leq \frac{n^j}{j!}$ trivially and that $\left[1 - \frac{2j}{n}\right]^{2k} = e^{2k \log \left(1 - \frac{2j}{n}\right)}$. Now we use the fact that $\log(1 - x) < - x $ for $|x| < 1$, to get that $e^{2k \log \left(1 - \frac{2j}{n}\right)} < e^{2k(- \frac{2j}{n})} = e^{-\frac{4jk}{n}}$ asymptotically. 
Therefore, 
\begin{equation}
	\left\| P^{\ast k} - U \right\|_{TV}^2 \leq \frac{1}{2} \sum_{j = 1}^{\left\lfloor \frac{n}{2} \right\rfloor}\frac{n^j}{j!} e^{-\frac{4jk}{n}}
\end{equation}
...
\begin{align*}
	\left\| P^{\ast k} - U \right\|_{TV}^2 \leq \frac{1}{2} \sum_{j = 1}^{\left\lfloor \frac{n}{2} \right\rfloor}\frac{1}{j!}e^{-jc}\\
...
\section{Conclusion}
Investigating statistical or combinatorial problems such as card shuffling can lead to interesting mathematics. In particular, if the problem space is nice, like on a group, then we can use tools such as representation theory and Fourier analysis on groups to build good bounds on how fast we converge to the uniform distribution. This report mainly focused on the representation theory part on this area of knowledge, though we do discuss the combinatorial aspects as well. 
\printbibliography
\end{document}
