...
In this paper, we introduce the concepts of random walks and specifically a random walk on a finite abelian group. We will define a random walk on a finite abelian group and then proceed with some important examples, but without proof as we do not discuss cutoff times in this report. We also discuss how a random walk on a group differs from a random walk on a set, namely the random walk respects the group structure, meaning that evolving a Markov chain through time is also a convolution on the elements of the group. We finally discuss some Fourier analysis to prove an upper bound on how far a limiting distribution is from the uniform distribution, thus proving that we can converge to the uniform distribution. 
