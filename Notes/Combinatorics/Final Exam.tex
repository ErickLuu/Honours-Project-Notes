...
We can write $M(x, y+1) = 1/y(M) = \frac{1}{y}(M)$. 
Then we can write $(y + 1) M(x, y + 1) = yD_y((M)/y) = M_y - M/y$. 
We have that to extract $n$ from $m(n, \ell + 1)n$, we take $xD_x$, so we have $ xM_x/y$. 

Finally, $m(n, \ell)\ell$ is just $yM_y$. 

Then this implies that:
\begin{align*}
	\frac{1}{y}\dfrac{\partial}{\partial x} M(x, y) &= M_y - M/y + xM_x/y + yM_y\\
	M_x &= y M_y - M + x M_x + y^2 M_y\\
	M = (x - 1) M_x + (y + y^2)M_y
\end{align*}
...
\begin{equation}
	M(x, 1) = (x - 1) A \log\left(\frac{1}{2(x - 1)}\right)
\end{equation}
Now 
