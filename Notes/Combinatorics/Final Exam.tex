...
\end{equation}
where $[x^n/n! y^k] H(x,y)$ is the number of ways to have unordered $k$ queues from $n$ people. But we want that these queues have a circular order, therefore we look at the queue with $1$ in it and make that one the start. Then there are $(k - 1)!$ queues, so we use the $y D_y$ operator on $H(x,y)$ and treat $H(x, y)$ like an egf on $y$. So we have that:
\begin{equation}
	G(x,y) = y D_y(H(x,y)) = \frac{e^{\frac{x y}{1 - x}}x y}{1 - x}
\end{equation}
To remove the $y$ term, we take:
\begin{equation}
	G(x) = \sum_{n \geq 0} D^n_y(G(x, y))|_{y = 0}.
\end{equation}
We have that $D^n_y(G(x, y))|_{y = 0} = \frac{n x^n}{(1 - x)^n}$. 
We have that the $k$-th coefficient of $\frac{1}{(1 - x)^n}$ is $\binom{n + k - 1}{k}$, so the $k$-th coefficient of $\frac{x^n}{(1 - x)^n}$ is $\binom{k - 1}{k - n}$ when $k > n$. 

\begin{equation}
	[x^n/n!] G(x) = n! \sum_{k = 1}^{n} k \binom{n - 1}{n - k}
\end{equation}
