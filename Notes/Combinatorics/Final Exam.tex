...
\begin{equation}
	H(x, y) = \frac{1}{(1 - x)^y}
\end{equation}
with $H(x, y)$ being an ops in $y$. Therefore, we have that $h(n, k) = k$. 
...
\end{equation}
By the same argument above, , then the generating function for the right-leaning side of the pyramid are the number of partitions of $n$ where each part has size at most $r$.
\begin{equation}
