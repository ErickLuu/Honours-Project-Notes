...
\end{equation}
where $Cyc(\sigma)$ is the number of cycles in $\sigma$. 

\subsection{b}
Now we will move onto solving this using inclusion-exclusion.
Let $P_N$ be the property that $N$ is not included in the matrix, $P_S$ be the property that $S$ is not included, and so on.
Let $F_k(n) = \frac{1}{|G|} \sum_{\sigma \in G} k^{n Cyc(\sigma)}$. 
We have that the ground set $|A| = \frac{1}{|G|} \sum_{\sigma \in G} 4^{n Cyc(\sigma)} = F_4(n)$.
We have that $\sum_{Y \subseteq S, |Y| = k} N_\geq(Y) = \binom{4}{k} F_{4 - k}(n)$. 
So we have that from Inclusion-Exclusion,

\begin{equation}
	N_{=}(\emptyset) = F_{4}(n) - 4 F_{3}(n) + 6 F_{2}(n) - 4 F_{1}(n) = \frac{1}{|G|} \left[ \sum_{\sigma \in G} 4^{n Cyc(\sigma)} - 4 \sum_{\sigma \in G} 3^{n Cyc(\sigma)} + 6 \sum_{\sigma \in G} 2^{n Cyc(\sigma)} - 4 \right]
\end{equation}
as $\frac{1}{n!} \sum_{\sigma \in G} 1^{n Cyc(\sigma)} = 1$. 
...
It is obvious that $L_n = 0$ when $n < n(n-1)/2$ as there are not enough logs to reach the top, and $L_{n(n-1)/2} = 1$. 

\section{Question 5}

Let $m(n, \ell)$ be the number of diagrams with $n$ people in $\ell$ lines.
We have that $m(n, 1) = n!$, which is the number of ways to place $n$ people in a single queue. 
We additionally have that $m(n,n) = (n-1)!$ which is the number of ways to place $n$ people around a circle with rotation invariant.

For a recursion, we have that:
\begin{equation}
	m(n,\ell) = m(n-1, \ell)(\ell + n - 1) + m(n-1, \ell - 1)(\ell - 1)
\end{equation}
when $n,\ell > 1$.
where we look at where the $n$-th person goes. If they are in their own queue, then we can remove them. There are $(\ell - 1)$ places they can go. If they are not in their own queue, they are either standing in front of someone or in the back of a line, so there are $\ell + n - 1$ places they can be standing. 

Finally, we have that:

\begin{equation}
	m_n = \sum_{\ell = 1}^{n} m(n, \ell)
\end{equation}

Now let $M(x, y)$ be a polynomial where $M$ is an egf in $x$ and an ops in $y$. We have that $[x^n y ]M(x, y) = 1$ for all $y$ by the law above, and that $[x^n y^n] M(x, y) = \frac{1}{n \times n!}$. 

Therefore, $m_n = [x^n/n!] M(x, 1)$. 

We also have that $[xy] M(x, y) = 1$. 

From the recursion above, we can write this as:
\begin{equation}
	m(n+1,\ell + 1) = m(n, \ell + 1)(\ell + 1) + m(n, \ell + 1)n + m(n, \ell)\ell
\end{equation}
when $n, \ell \geq 1$, and $(0, \ell) = 0$, $m(n, 0) = 0$ for all $n, \ell$.


We have that $(y) M(x, y) = yD_y((M)) $. The $(\ell + 1)$-th term is $ (yD_y(M) - 1)/y$. 
We have that to extract $n$ from $m(n, \ell + 1)n$, we take $xD_x$ of $M/y$, so we have $ xM_x/y$. 

Finally, $m(n, \ell)\ell$ is just $y D(M) = y M_y$. 

Then this implies that:
\begin{align*}
	\frac{1}{y}(\dfrac{\partial}{\partial x} M(x, y)) &=  (yD_y(M) - 1)/y + xM_x/y + yM_y\\
	1/y M_x  &= M_y - 1/y + x M_x/y + y M_y\\
	M_x &= x M_x - 1 + (y + y^2) M_y\\
	0 &= (x - 1) M_x + (y + y^2) M_y - 1
	M = (x - 1) M_x + (y + y^2)M_y + M(x,0)
\end{align*}
so using a PDE solver, we have that:

\begin{equation}
	M(x,y) = A(1-x)\left(\log(y) - \log(1 + y) - \log(x - 1)\right)
\end{equation}
Therefore, $M(x, 1)$ is the coefficient that we need, which is
\begin{equation}
	M(x, 1) = A(1-x)
\end{equation}
Now 
