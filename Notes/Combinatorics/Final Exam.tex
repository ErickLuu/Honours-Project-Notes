...
\subsection{c}
We will evaluate the integral $[z^n] f(z) = \oint_C f(z)/z^{n + 1} dz =  \int_{-\pi}^{\pi} g(\theta) \, dx$ where $ g(\theta) := \frac{f(r e^{i \theta})}{r^n e^{i n \theta}}$.
...
\begin{align*}
	...
	&= \frac{e^{r e^r}}{r^n \sqrt{r e^r(r + (r + 1)^2)}}
\end{align*}
Inserting $r(r + 1) e^r = n$, we have that
\begin{equation}
	[z^n] f(z) = \frac{e^{r e^r}}{r^{r (r + 1) e^r} \sqrt{r e^r(r + (r + 1)^2)}}
\end{equation}
