...
\begin{equation}
	...
	{2 \pi(\sqrt{n} + y) }\\
\end{equation}
We have that $2 \pi (\sqrt{n} + y) \sim 2 \pi \sqrt{n}$ uniformly with a small enough $y$. 
...
To verify this answer, consider
\begin{equation}
	h(z) = \frac{e^{-z/4}}{(1 - z)^{1/4}}.
\end{equation}
We have that $f(z) = h(z^2)$, therefore by the calculus of egfs, we have that the entries of $f$ are the same as the entries of $h$ except padded out by one zero between each coordinate. We have that:
