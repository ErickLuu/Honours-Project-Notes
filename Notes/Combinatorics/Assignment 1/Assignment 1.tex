...
\end{equation}
but as we have that $N_{\geq}(S_4) = N_{\geq}(S_4) = m_1$, and $N_{=}(S_0) = m_0$, then we can write this equation as:
\begin{equation}
...
\begin{equation}
	m0 - m1 = \binom{n}{2}\binom{n}{2} - kn(n-1)^2 + (2n \binom{k}{2}(n-1) + \frac{nk(nk - 2k + 1)}{2}) - 2 n \binom{k}{2} (k - 1)
\end{equation}
\end{document}
