...
\begin{frame}[allowframebreaks]{Book-Embedding}
	\begin{figure}[h]
		...
		\begin{minipage}{0.5\textwidth}
			\begin{block}{Books}
				A book is an embedding of a graph where the vertices live on the spines and the edges live on pages and do not cross.
			\end{block}
		\end{minipage}
	\end{figure}
...
\begin{frame}
	...
	\end{theorem}
	\begin{theorem}[Malitz]
	Graphs of genus $g$ have pagenumber $O(\sqrt{g})$. 
\end{theorem}

Moral: "Global" bounded graph structures generally have bounded pagenumber.
\end{frame}
