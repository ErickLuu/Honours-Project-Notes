...
\begin{theorem}
	For all minor-closed classes $\mathcal{G}$, $\mathcal{G}$ has bounded rtw iff some apex graph $A$ is not in $\mathcal{G}$.
\end{theorem}
...
Then $G$ is $X$-minor-free iff $G$ can be constructed out of 
\begin{itemize}
	\item surfaces
	\item vortices
	\item weak apex vertices
...
Then the shortcut system $G_0^\mathcal{P}$ is the graph where $V(G_0^\mathcal{P}) = V(G_0)$ and $vw \in E(G_0^\mathcal{P})$ iff there is a $vw$ path in $P$. We claim that for all $k$-planar graphs $G$, there is a planar graph $G_0$ and $G = G_0^\mathcal{P}$ for some $(k + 2, 2)$-shortcut system. 
