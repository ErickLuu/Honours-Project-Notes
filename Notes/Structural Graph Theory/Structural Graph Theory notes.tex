...
\begin{theorem}
	If $G$ is a graph and for all $r$ $\nabla_r(G) \leq c r^d$. then every $n$-vertex subgraph of $G$ has a balanced separator of order $\leq c' n^{1 - \varepsilon}$ where $\varepsilon = \varepsilon(d) > 0$ and is roughly $1/d$. Therefore, $\tw(G) \leq O(n^{1- \varepsilon})$ and we say that $\tw(G)$ has a strongly sublinear separator. 
\end{theorem}
\end{document}
