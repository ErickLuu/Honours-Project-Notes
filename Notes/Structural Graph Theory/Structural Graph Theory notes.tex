...
We define the genus $g$ of a surface to be 2 times the number of handles + the number of crosscaps. Thus we have that the Euler characteristic $\chi = 2 - g$ for both orientable and non-orientable surfaces. 

In fact, we can show that if $G$ has Euler genus $g$, then if $G$ has $n$ vertices and $m$ edges, then $n - m + f = \chi = 2-g$, therefore we can show that 
\begin{theorem}{Clique-sum}
	content...
\end{theorem}
