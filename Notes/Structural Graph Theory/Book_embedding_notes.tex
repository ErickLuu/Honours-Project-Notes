...
The Euler genus of a surface $\Sigma$ obtained from a sphere by adding $h$ handles, $c$ crosscaps and $t$ twisted handles is $2h + 2t + c$.
...
\subsubsection{Euler Genus-g graphs}\label{sssec:Graph_genus}

We define the \textit{Euler Genus} of a graph $G$ to be the smallest Euler genus $g$ surface $\Sigma$ such that $G$ can be embedded on $\Sigma$ without crossings (note that $\Sigma$ can be orientable or nonorientable).
