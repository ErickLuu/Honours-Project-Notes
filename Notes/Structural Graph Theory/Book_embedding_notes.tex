...
We aim to solve the question using the graph structure theorem \cite{robertsonGraphMinorsXVI2003}, which describes the layout of graphs which do not contain a $K_t$ minor. It states that all graphs that do not contain a $K_t$ minor can be built out of \textit{clique sums} of graphs which are $(g, p, k, a)$- almost embeddable. These are graphs which can be embedded on a surface of bounded genus $g$, with some \textit{vortices} which cannot be embedded on the surface, but have bounded width. We have that all graphs of bounded genus have bounded pagenumber \cite{heathPagenumberGenusGraphs1992}, all graphs of bounded treewidth have bounded pagenumber \cite{ganleyPagenumberTrees2001}, and finally clique-sums of graphs with bounded pagenumber on cliques of bounded size have bounded pagenumber(cite here). 
