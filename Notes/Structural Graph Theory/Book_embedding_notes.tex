...
Treewidth was introduced in \cite{berteleChapterEliminationVariables1972}
...
We define the genus $g$ of a surface to be 2 times the number of handles + the number of crosscaps. From topology, we know that the connected sum of the torus $T$ and the projective plane $\mathbb{R}P^2$ is diffeomorphic to the connected sum of three copies of $\mathbb{R}P^2$, thus we can make any surface with crosscaps and handles to be purely crosscaps. Therefore, the Euler characteristic $\chi = 2 - g$ for both orientable and non-orientable surfaces. Note that the genus is defined slightly differently from topology. We do this to allow non-orientable and orientable surfaces to coincide in definition.
...
We additionally have that $\pn(G) \leq \twn(G) + 1$, from \cite{ganleyPagenumberTrees2001}. 
