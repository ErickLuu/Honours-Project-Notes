...
A \textit{model} of a graph $H$ to a graph $G$ is a function $\rho$ which assigns to $H$ vertex disjoint connected subgraphs of $G$, such that if $uv \in E(H)$, then some edge in $G$ connects a vertex in $\rho(u)$ to $\rho(v)$.
...
\begin{theorem}
	...
	We shall prove the contrapositive. Suppose $G + a$ has a $K_{t + 1}$ minor. Then suppose we remove $a$ in the $K_{t + 1}$ minor, meaning we delete the vertex which $a$ has been contracted into. Then we have 
\end{theorem}
