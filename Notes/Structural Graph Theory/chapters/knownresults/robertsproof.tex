...
\begin{lemma}\label{lem:planar_graphs_4_connected_cliqesums}
	Every planar graph \(G\) with at least 5 vetices has a tree-decomposition such that each torso is a \(4\)-connected planar graph (or a subgraph of $K_4$) with adhesion at most \(3\).
\end{lemma}
