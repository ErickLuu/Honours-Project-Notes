...
\begin{definition}[Euler Genus]
	The \textit{Euler genus} of a surface \(\Sigma\) obtained from a sphere by adding \(h\) handles, \(c\) crosscaps is\(2h + c\).
\end{definition}
...
\begin{theorem}[Bounds on Graph Minor Structure Theorem by \textcite{joretCompleteGraphMinors2013}:]\label{thm:graph_structure_bound_theorem}
	For all graphs \(G\),
