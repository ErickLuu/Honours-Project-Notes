...
\begin{lemma}
	Let $\Sigma$ be a surface and let $x_0$ be a point on the surface. Then let $L$ be nontrivial loops that start and end at $x_0$ on $\Sigma$. Then let $F_1, ..., F_j$ be faces on $\Sigma + L$, such that each $F_i$ is homeomorphic to a disk. Then the length of the facial walk for all $F$ is at most $2g$. 
\end{lemma}

Parts of the proof was motivated by a discussion with Corbin Reid. 

\begin{proof}
	Let $G'$ be the dual graph of this graph, where the vertex set is $F_1, ..., F_j$ and the edge set are the loops, where two faces are incident if there is a loop that is touching both faces. Then take a spanning tree of the graph, and let $L'$ be the loops that are not in the spanning tree and $L''$ be the loops that are in the spanning tree. Then consider embedding $L'$ on the surface $\
\end{proof}

\begin{corollary}
	From the theorem above, every nonplanar face alternates between having a planar trace and nonplanar edge at most $2g$ times. Therefore, we are bounded.
\end{corollary}
