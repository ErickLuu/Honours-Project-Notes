...
Preserved faces are very important. Let $G$ is a graph embedded on a surface with a vortex $G'$ with pathwidth $\leq k$ on a face $F$. We also have a book-embedding of $G - G'$ which preserves $F$. Then by \cref{thm:bded_treewidth_bded_pagenumber} we need only $k + 1$ extra pages to add $G'$. This is because we 
