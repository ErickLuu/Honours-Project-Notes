...
If a graph $G$ has a path-decomposition \({(B_i)}_i\), then $G$ has a tree-decomposition \(\left({(B_i)}_i, P\right)\). Therefore,
\begin{equation*}
	\pw(G) \geq \tw(G).
\end{equation*}
...
\begin{example}
	Recall that $K_n$ is the complete graph on $n$ vertices. \(\pw(K_n) = \tw(K_n) = n - 1\).
\end{example}
\begin{proof}
