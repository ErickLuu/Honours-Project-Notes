...
\begin{proof}
	...
	$\Rightarrow$ Suppose \(G\) has pathwidth 1. Then for each connected component of \(G\), choose a vertex \(v\) in \(B_1\) and a vertex \(w\) in \(B_n\), the final bag, and look at a path from \(v\) to \(w\). This path must go through every bag, and furthermore every vertex not on this path is adjacent to a single vertex on this path. So $G$ is a tree, and therefore $G$ is a caterpillar. 
