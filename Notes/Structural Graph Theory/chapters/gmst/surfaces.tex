...
This section goes into more detail about graphs on surfaces. We first discuss surfaces in more detail, then discuss graphs embedded on a surface.

\subsection{Surfaces}

The terminology in this section is based on \textcite{moharGraphsSurfaces2001}. Recall a surface is a compact $2$-manifold. Recall also that every surface is homeomorphic to a sphere with $h$ handles and $c$ crosscaps, for some integer $h$ and $c$. This is the classification of surfaces. The \textit{Euler genus} of a surface \(\Sigma\), obtained from a sphere by adding \(h\) handles and \(c\) crosscaps, is \(2h + c\).
...
Note that ``genus'' and ``Euler genus'' are two distinct concepts. In many works, ``genus'' refers to the orientable genus. However, in this thesis, genus is a synonym \underline{Euler genus} unless stated otherwise.
