...
Graphs embeddable on a surface of Euler genus $g$ are $K_t$ minor-free, where \(t > \sqrt{6g} + 4\). This comes from \cref{thm:bounded_genus_kt_free}. In the case when the surface is a torus, $K_7$ is a toroidal graph but $K_8$ is not. An example of an embedding of $K_7$ on a torus is in \cref{fig:k7_on_torus}.
...
\end{proposition}
\begin{proof}
	Firstly, $k < t$. If $G$ has a clique of size $t$ then $G$ has a $K_t$ minor. Let $C = V(G) \cap V(H)$ be the clique that is being pasted over. Furthermore, suppose that no edges in $C$ were deleted when pasting cliques. Adding more edges does not remove a $K_t$ minor. 
