...
\begin{theorem}[\textcite{kawarabayashiQuicklyExcludingNonplanar2021}]
	Let $t \geq 1$ be a positive integer. Let $G$ be a $K_t$-minor free graph. Then let $\alpha = t^{18 \cdot 10^{7} t^{26} + c}$ for a constant $c$. Then setting $g = t(t+1)$, $p = 4\alpha$, $k = \alpha$ and $a = 3\alpha$, $G \in \mathcal{G}(g,p,k,a)$. 
\end{theorem}
