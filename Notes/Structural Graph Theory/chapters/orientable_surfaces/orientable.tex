...
\begin{theorem}\label{thm:orientablevortices}
	Let $G$ be a graph with a book-embedding where all torsos are $k$ almost-embeddable on an orientable surface of genus at most $g$. Now suppose there are $p$ vortices of width $k$ which are embedded on each torso of $G$. Then $G$ can be embedded on $(2s + 2)(s) + \ell$ pages, where $s = \gpk$. 
\end{theorem}
