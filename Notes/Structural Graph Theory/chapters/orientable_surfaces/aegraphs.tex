...
To solve this problem, we define some new concepts. Let \(G\) be a graph. A vertex ordering \((<)\) \textit{preserves} a face \(F\) if $<$ restricted to the vertices on the boundary of $F$ maintains the natural order when traversing the boundary of the face.
...
\begin{proof}
	...
	\end{figure}
\end{proof}
From \textcite{tutteTheoremPlanarGraphs1956}, if \(G\) is a 4-connected planar graph, then the vertex ordering of the Hamiltonian cycle \((\leq)\) preserves all faces on \(G\). As a consequence, every 4-connected planar graph has a circular ordering which preserves every face.
