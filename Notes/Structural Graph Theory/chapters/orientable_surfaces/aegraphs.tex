...
A circular ordering \(<\) preserves a face \(F\) if the induced linear ordering has the condition above. 
Let \(F\) be a face on \(G\). Let \( (<, \varphi) \) be a book-embedding of \(G\). A \textit{monochromatic path} $P$ on the boundary of $F$ is a maximal path where every edge is in the preimage of some colour in $\varphi$. $F$ being a preserved face implies that $F$ has a single monochromatic path, but not the other way around. 
...
\begin{enumerate}
	\item Suppose $G$ is a \(4\)-connected planar graph. There is a book-embedding where every book is monochromatic. 
