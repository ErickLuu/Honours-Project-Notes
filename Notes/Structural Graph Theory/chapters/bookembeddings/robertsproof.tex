...
Let $G$ be a plane graph. A \textit{separating triangle} on $G$ is a triangle $u,v,w$ on $G$ such that the triangle $uvw$ separates $\mathbb{R}^2$ into two components, and $G - \{uvw\}$ is nonempty on each component.
\todo{draw picture!}
