...
For a subset of vertices \(A \subseteq V(G)\), we denote the \textit{induced subgraph} on \(G\) with vertex set \(A\) as \(G[A]\). The neighborhood of the vertices $A$ is defined to be all vertices that are adjacent to a vertex in $A$ and not in $A$ and is denoted as $N(A)$. 
