...
\end{conjecture}
However, the outline given in their paper is insufficient to prove \cref*{lem:planar_nonplanar_nonorientable}\todo{should there be a discussion of their outline? because if its incomplete then showing that it is incomplete might be trying to prove a negative}. In fact, their proof method fails when a graph is projective-planar, as stated by \textcite{nakamotoBookEmbeddingProjectiveplanar2015} and \textcite{ozekiBookEmbeddingGraphs2019}. An example of a projective-planar graph is in \cref{fig:projectiveplanar}, and using the algorithm laid out by \textcite{heathPagenumberGenusGraphs1992} does not yield a book-embedding of this graph. 
