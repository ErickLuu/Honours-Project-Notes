...
If $p = 0$, then this is an issue. 

\section{7}

\subsection{1}
Let $X, Y$ be two handlebodies of genus $0$. Then $X$ and $Y$ are $2$-balls and are therefore homeomorphic. Now suppose $X$ and $Y$ are two handlebodies of genus $g$. 
...
\subsection{3}
Let $M = (H_1, H_2, f)$ be a Heegaard splitting of $M$ into handlebodies $H_1$, $H_2$ of genus $g$ with a homeomorphism $f$ from $\partial H_1 \rightarrow \partial H_2$. 
The fundamental group of a genus $g$ handlebody is $F_g$, by taking the homotopy of every handle to a point, yielding $g$ wedges of circles. Using Seifert Van-Kampen, the fundamental group of $M$ is $\pi_1(H_1) * \pi_1(H_2)/N_f$ where $N_f$ is the normal subgroup of identity loops. 

As $f$ is a homeomorphism, the map $f_*$ that takes loops in $\pi_1(\partial H_1)$ to $\pi_1(\partial H_2)$ must be an isomorphism by the definition of the fundamental group. As $\pi_1(H_1)\leq \pi_1(\partial H_1)$, then the map $f': \pi_1(H_1) \rightarrow \pi_1(H_2)$ is also an isomorphism. Therefore, the elements $\{g_1, ..., g_n\}$ corresponding to meridinal loops of $\pi_1(H_1)$ has a corresponding basis $\{\ell_1, ..., \ell_n\}$ in $\pi_1(H_2)$. Furthermore, $g_1 \ell_1^{-1}$ is the trivial loop on $\partial H_1$, so is in $N_f$. Therefore, the fundamental group of $M$ is $\langle g_1, ..., g_m | R \rangle$ where $R$ are relations derived from taking the meridians of $H_2$ to $\{\ell_1, ... \ell_g\}$. 

\subsection{4}
The contrapositive of question $3$ is that if the fundamental group of $M$ has at $g$ generators, then $M$ has genus at least $g$. 

The fundamental group of $T^3$ has no presentation with two generators, as the fundamental group of $T^3$ is $\mathbb{Z} \times \mathbb{Z} \times \mathbb{Z}$. Since $(1, 0, 0)$, $(0, 1, 0)$, and $(0,0, 1)$ are linearly independent in this group, then every representation must have at least three generators. By question3, the genus must be at least 3. 
