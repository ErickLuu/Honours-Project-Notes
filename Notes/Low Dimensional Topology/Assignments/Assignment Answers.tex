...
\section{5}
\subsection{1}
There exists a homeomorphism from $\alpha$ to the side of a fundamental polygon. Now there exists a cycle that passes through $\alpha$ once that 
\subsection{2}
\subsubsection{a}
Let $\alpha, \alpha'$ and $\beta, \beta'$ be isotopic with isotopies $h_\alpha$ and $h_\beta$. Firstly, $h_\beta$ maps $\beta$ to $\beta'$ along a strict homeomorphism. For every intersection in $\alpha$ and $\beta'$, when the homotopy $h_\beta$ goes back in time, either the intersection was from $\beta$ or induced by a new crossing at some point. However, the only way that new intersections can be introduced if they are added by some bigon, therefore $\inter(\alpha, \beta')$ restricted to the bigon is going left and right and therefore must add to 0. Repeating this argument for every crossing, it must hold that $\alpha$ and $\beta'$ cross either when $\alpha$ crosses $\beta$ or when a bigon is introduced. Therefore, $\inter(\alpha, \beta') =  \inter(\alpha, \beta)$. Now repeat this argument for $\inter(\alpha, \beta')$ and $\inter(\alpha', \beta')$ but traverse along $\beta'$ instead, and use subsection $b$. 
...
If $a$ and $b$ are essential and do not intersect, consider $S - \{a\}$. Then one of the components of $S - \{a\}$ will contain $b$. Now this is a surface with $k$ holes, and $b$ is an essential curve on this component of $S - \{a\}$. Then removing $b$ from this component will yield some components. No component is an annulus nor disk, as $b$ is essential. Then take a curve $c$ which goes around one hole in $b$ and then goes around a hole not in $b$ on $S - \{a\}$. This will intersect with $b$ but will not intersect with $a$. Furthermore, as it goes around a hole in $b$, then $i(b, c)$ is not zero. 
\subsubsection{b}
Let $c$ be a curve which follows $a$ but does not cross $a$. Then $c$ will cross $b$ near where $a$ crosses $b$. Thus shown. 

\subsubsection{c}
Suppose $a$ and $b$ do not cross. Take $c$ to be the curve from part $a$. Then $i(T_a(c), c) = 0$ as $T_a(c) \sim c$ up to ambient isotopy, since $T_a(c)$ is just $c$ but pushed to the side. But $i(T_b(c), c)$ is not 0, as $c$ crosses $T_b(c)$. Let $x, y$ be two crossings in $c \cap b$. Consider the twist at $x$. It wraps around and returns to $x$. But then this twist must pass by $y$, which is a crossing in $x$. Therefore, $T_b(c)$ must cross $c$.
FINISH LATER. 
\subsection{d}
...
\paragraph*{i}
If $T_a T_b a = b$ is equivalent to $T_{T_a T_b(a)} = T_b$. But this is equivalent to $(T_a T_b) T_a (T_a T_b)^{-1} = T_b$ from the definition of conjugation in exercise $5$. But this is equivalent to $T_a T_b T_a = T_b T_a T_b$. Therefore, this satisfies the braid relation. 

\paragraph*{ii}
If $T_a T_b a = b$, then $i(a, T_b(a))$ is 0 outside of a neighbourhood of $b$. However, inside of this neighbourhood, at every point in $b \cap a$, the Dehn twist $T_b(a)$ will intersect at every point near $b \cap a$. Therefore, $i(a, T_b(a)) = i(a, b)$. Therefore, $i (a, b) = i(a, T_b(a)) = i(a, b)^2$ from exercise 3. Therefore, $i(a, b) = 1$.
\paragraph*{iii}
$T_a T_b a = b$ if and only if $i(a, b) = 1$ and $T_a T_b a = b$ if and only if $T_a T_b T_a = T_b T_a T_b$. Therefore, $i(a, b) = 1$ if and only if $T_a T_b T_a = T_b T_a T_b$.
