...
\subsection{6}
Firstly, $A$ is homeomorphic to $B^3$ with $n$ disjoint cylinders drilled out. Now $B^3$ with a single cylinder drilled out deformation retracts to $S^1$. Now use Seifert Van Kampen on $B$. Take $S_i$ to be $B$ but excluding a closure of a neighbourhood around every cylinder except for the $i$-th cylinder. Then the pairwise overlap of $S_i \cap S_j$ is a $3$-ball, so every overlap yields no relators. Furthermore, $\pi_1(S_i) = \mathbb{Z}$ with generator the loop that goes around the cylinder, which is $x_i$ in $A$. Furthermore, $B = \bigcup_{i = 1}^n S_i$. Using Seifert-Van Kampen, $\pi_1(A) = *_{i = 1}^n S_i$ which is the free group on $n$ generators. Furthermore, the generators are $x_1, \ldots, x_n$. 
\end{document}
